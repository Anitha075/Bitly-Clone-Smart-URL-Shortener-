...
const Navbar = () => {
  ...
  // Temporary values until Context API is fixed
  const token = null;
  const setToken = () => {};
    ...
    <div className="h-16 bg-custom-gradient z-50 flex items-center sticky top-0">
      <div className="lg:px-14 sm:px-8 px-4 w-full flex justify-between items-center">
        ...
        <ul
          ...
          } transition-all duration-100 sm:h-fit bg-custom-gradient sm:w-fit w-full sm:flex-row flex-col px-4 sm:px-0`}
        >
          ...
          {!token && (
            <li>
              <Link
                to="/register"
                className="bg-rose-700 text-white cursor-pointer w-24 text-center font-semibold px-2 py-2 rounded-md hover:text-slate-300 transition-all duration-150 block"
              >
                SignUp
              </Link>
            </li>
